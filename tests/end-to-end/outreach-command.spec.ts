...
test("authorized demo operator opens the morning brief and intelligence controls", async ({ page }) => {
  await page.goto("/login");
  await expect(page.getByRole("heading", { name: /Turn public questions into respectful ministry action/ })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Google Business Profile eligibility gate" })).toBeVisible();
});
