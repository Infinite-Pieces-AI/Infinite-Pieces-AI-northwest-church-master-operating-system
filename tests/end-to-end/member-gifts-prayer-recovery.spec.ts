...
test("member can explore Gifts of the Church, Prayer Well, and Recovery Ministry showroom", async ({ page }) => {
  await page.goto("/gifts");
  await expect(page.getByRole("heading", { level: 1, name: "Use what God has given you" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { level: 1, name: "Carry one another in prayer" })).toBeVisible();
  await page.getByRole("button", { name: "Add request" }).click();
  await page.getByLabel("Short title").fill("Wisdom for the week");
  await page.getByLabel("Prayer request").fill("Please pray for wisdom, humility, and patience this week.");
