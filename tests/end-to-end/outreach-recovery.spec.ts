...
test("authorized showroom operator can use Recovery Outreach without individual profiling", async ({ page }) => {
  await page.goto("/recovery-outreach");
  await expect(page.getByRole("heading", { level: 1, name: "Recovery Outreach" })).toBeVisible();
  await expect(page.getByText(/Help people find responsible support without profiling private lives/i)).toBeVisible();
  ...
  await expect(page.getByText(/Hub recovery posts, progress, attendance, or membership/)).toBeVisible();
});
