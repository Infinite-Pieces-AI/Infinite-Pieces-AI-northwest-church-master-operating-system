...
describe("invitation tokens", () => {
  it("stores a different hash rather than the raw token", () => {
    ...
    expect(
      invitationTokenMatches({
        ...
        expectedHash: invitation.tokenHash,
        pepper,
      }),
    ).toBe(true);
  });
  ...
  it("rejects used, revoked, and expired invitations", () => {
    ...
    expect(
      isInvitationUsable({ expiresAt: new Date("2026-08-05T12:00:00Z"), usedAt: now, now }),
    ).toBe(false);
    expect(
      isInvitationUsable({ expiresAt: new Date("2026-08-05T12:00:00Z"), revokedAt: now, now }),
    ).toBe(false);
  });
});
