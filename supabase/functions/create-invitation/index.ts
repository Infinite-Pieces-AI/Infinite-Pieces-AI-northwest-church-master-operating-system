...
Deno.serve(async (request) => {
  ...
  if (!url || !anonKey || !serviceRole || !pepper) return jsonResponse({ message: "Invitation service is not configured" }, 503);
  ...
  if (!email || !email.includes("@") || roles.length > 8) return jsonResponse({ message: "Invalid invitation request" }, 400);
  ...
  if (error) return jsonResponse({ message: "Invitation could not be created" }, error.code === "42501" ? 403 : 400);
  ...
  if (inviteError) {
    ...
    if (!emailWebhook) {
      if (!localTokenReturn) {
        ...
        return jsonResponse({ message: "Invitation delivery failed and the invitation was revoked" }, 502);
      }
  ...
  return jsonResponse({
    ...
    expiresAt,
    deliveryMode,
    ...(localTokenReturn ? { localDevelopmentAcceptanceUrl: `${appUrl.replace(/\/$/, "")}${nextPath}` } : {})
  }, 201);
});
