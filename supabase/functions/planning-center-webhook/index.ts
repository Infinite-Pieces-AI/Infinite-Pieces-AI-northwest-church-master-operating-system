 ...
 */
async function verifyProviderSignature(rawBody: string, suppliedSignature: string | null): Promise<boolean> {
...
Deno.serve(async (request) => {
  ...
  if (!signatureValid) return jsonResponse({ message: "Webhook verification adapter is not configured or signature is invalid" }, 401);
  ...
  if (!url || !serviceRole) return jsonResponse({ message: "Webhook storage is not configured" }, 503);
