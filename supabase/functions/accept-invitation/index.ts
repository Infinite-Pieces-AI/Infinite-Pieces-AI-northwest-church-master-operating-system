...
Deno.serve(async (request) => {
  ...
  const body = await request.json().catch(() => null) as { token?: string; privacyAccepted?: boolean; communityGuidelinesAccepted?: boolean } | null;
  if (!body?.token || body.token.length < 32 || !body.privacyAccepted || !body.communityGuidelinesAccepted) {
  ...
  if (!url || !anonKey || !pepper) return jsonResponse({ message: "Invitation service is not configured" }, 503);
  ...
  if (error) return jsonResponse({ message: "Invitation is invalid, expired, revoked, used, or belongs to another email" }, 400);
