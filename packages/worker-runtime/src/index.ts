...
export function requireServerEnvironment(): {
  ...
  if (!url || !serviceRoleKey)
    throw new Error("Workers require server-only SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.");
  if (process.env.NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY)
    throw new Error("A service-role key must never use a NEXT_PUBLIC_ prefix.");
...
export async function runWorker(
  ...
  const log = (event: string, details: Record<string, unknown> = {}) => {
    process.stdout.write(
      `${JSON.stringify({ timestamp: new Date().toISOString(), workerName, runId, event, dryRun, ...details })}\n`,
    );
  };
  log("worker.started");
  try {
...
export async function claimOutboxEvents(
  ...
  Array<{ id: string; event_type: string; payload: Record<string, unknown>; attempts: number }>
> {
...
export async function failOutboxEvent(
  ...
  const { error } = await context.supabase.rpc("fail_outbox_event", {
    requested_id: id,
