...
describe("web-push endpoint boundaries", () => {
  it("accepts an approved provider endpoint", () => {
    expect(validatePushEndpoint("https://fcm.googleapis.com/fcm/send/synthetic").hostname).toBe("fcm.googleapis.com");
  });
  ...
  it("supports an explicit deployment allowlist", () => {
    ...
    expect(validatePushEndpoint("https://tenant.notify.example/push/1", hosts).hostname).toBe("tenant.notify.example");
  });
