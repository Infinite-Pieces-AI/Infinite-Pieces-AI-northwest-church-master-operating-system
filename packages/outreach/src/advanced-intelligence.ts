...
export interface MinistryOpportunityContribution {
  key: keyof Omit<MinistryOpportunityInputs, "topic" | "sensitivityRisk" | "confidence" | "source" | "dateRange">;
  label: string;
  input: number;
  weight: number;
  contribution: number;
...
export function scoreMinistryOpportunity(input: MinistryOpportunityInputs): MinistryOpportunityAssessment {
  const contributions = (Object.keys(weights) as Array<keyof typeof weights>).map((key) => {
    const normalized = clamp(input[key]);
    const weight = weights[key];
    return { key, label: labels[key], input: normalized, weight, contribution: Number((normalized * weight).toFixed(2)) };
  });
  const weightedBase = Number(contributions.reduce((sum, item) => sum + item.contribution, 0).toFixed(2));
  ...
  const explanation = contributions
    ...
    .slice(0, 5)
    .map((item) => `${item.label}: ${item.input}/100 contributed ${item.contribution.toFixed(1)} points.`);
  if (riskPenalty > 0) explanation.push(`Sensitivity and policy risk reduced the score by ${riskPenalty.toFixed(1)} points.`);
  const recommendedActions = priority >= 80
    ? ["Review the source evidence today", "Improve or create the people-first public answer", "Attach a voluntary next step", "Assign a named human reviewer", "Measure aggregate outcomes"]
    : priority >= 60
      ? ["Verify the content gap", "Improve the relevant public page", "Monitor the next reporting period"]
...
export function evaluateBusinessProfileEligibility(input: BusinessProfileEligibilityInput): {
  ...
  return {
    status: missing.length === 0 ? "eligible_for_submission_review" : completionPercent >= 70 ? "review_required" : "blocked",
    completionPercent,
    missing,
  };
}
