...
const input = {
  ...
  households: [
    { id: "h1", displayLabel: "Household 1", memberCount: 2, memberIds: ["m1", "m2"], availability: ["tuesday"], lifeStage: "families" },
    { id: "h2", displayLabel: "Household 2", memberCount: 1, memberIds: ["m3"], availability: ["tuesday", "thursday"], isNewcomer: true },
    { id: "h3", displayLabel: "Household 3", memberCount: 2, memberIds: ["m4", "m5"], availability: ["thursday"], lifeStage: "families" },
    { id: "h4", displayLabel: "Household 4", memberCount: 1, memberIds: ["m6"], availability: ["tuesday", "thursday"] }
  ],
  groups: [
    { id: "g1", name: "Tuesday Group", minimumMembers: 2, maximumMembers: 4, availability: ["tuesday"], leaderHouseholdIds: ["h1"] },
    { id: "g2", name: "Thursday Group", minimumMembers: 2, maximumMembers: 4, availability: ["thursday"], leaderHouseholdIds: ["h3"] }
  ],
...
describe("generateRotationProposal", () => {
  it("is deterministic for the same seed and keeps households intact", () => {
    ...
    expect(new Set(first.assignments.map((item) => item.householdId)).size).toBe(input.households.length);
  });
  ...
  it("reports an infeasible proposal instead of silently violating capacity", () => {
    const proposal = generateRotationProposal({
      ...input,
      households: [...input.households, { id: "h5", displayLabel: "Large household", memberCount: 7, memberIds: ["a", "b", "c", "d", "e", "f", "g"], availability: ["tuesday"] }]
    });
