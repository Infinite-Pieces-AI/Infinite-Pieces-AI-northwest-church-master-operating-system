  ...
  for (const event of events) {
    try {
      ...
      if (event.event_type === "ai.draft_requested") {
        ...
        context.log("ai.embedding_guardrails", {
          eventId: event.id,
          approvedDocumentCount: approvedDocumentIds.length,
        });
      } else if (event.event_type === "curriculum.draft_requested") {
        const weeklyLessonId =
          typeof event.payload.weekly_lesson_id === "string"
            ? event.payload.weekly_lesson_id
            : null;
        ...
        const draft = buildImagePromptDraft({
          ...
          intendedUse: (event.payload.intended_use ?? "sermon_series") as
            "bible_tab" | "public_website" | "social_media" | "event" | "sermon_series",
          brandNotes: Array.isArray(event.payload.brand_notes)
            ? event.payload.brand_notes.filter(
