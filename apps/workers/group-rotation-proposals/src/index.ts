...
function parseRotationInput(value: unknown): RotationInput {
  ...
  if (!input.cycleId || !input.seed || !Array.isArray(input.households) || !Array.isArray(input.groups) || !Array.isArray(input.pairingHistory)) {
...
await runWorker("group-rotation-proposals", async (context) => {
  ...
  for (const event of events) {
    try {
      ...
      const requestedBy = typeof event.payload.requested_by === "string" ? event.payload.requested_by : null;
      if (!cycleId || !requestedBy) throw new Error("Rotation request requires cycle_id and requested_by");
      const input = parseRotationInput(event.payload.input_snapshot);
      if (input.cycleId !== cycleId) throw new Error("Rotation input cycle does not match event cycle");
      ...
      if (!context.dryRun) {
        ...
        if (proposal.assignments.length) {
          const { error: assignmentError } = await context.supabase.from("rotation_assignments").insert(
      ...
      await failOutboxEvent(context, event.id, error instanceof Error ? error.message : "Rotation proposal failed");
    }
  }
