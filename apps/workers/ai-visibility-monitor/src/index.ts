...
await runWorker("ai-visibility-monitor", async (context) => {
  ...
  for (const event of events) {
    ...
    try {
      ...
      const providerKey = typeof event.payload.provider_key === "string" ? event.payload.provider_key : "approved-proxy";
      if (!context.dryRun) {
      ...
      const response = await fetch(proxy, {
        ...
        body: JSON.stringify({ prompts, publicSourcesOnly: true, noPersonalization: true, returnCitations: true, returnOtherOrganizations: true }),
      ...
      const rows = (payload.results ?? []).slice(0, prompts.length).flatMap((result) => {
        ...
        const organizations = Array.isArray(result.otherOrganizations)
          ? result.otherOrganizations.filter((item): item is string => typeof item === "string").map((item) => item.slice(0, 160)).slice(0, 20)
          : [];
        const citedPath = typeof result.citedPagePath === "string" && result.citedPagePath.startsWith("/")
          ? result.citedPagePath.slice(0, 500)
          : null;
        return [{
          run_id: runId,
          prompt,
          provider_name: typeof result.providerName === "string" ? result.providerName.slice(0, 120) : providerKey,
