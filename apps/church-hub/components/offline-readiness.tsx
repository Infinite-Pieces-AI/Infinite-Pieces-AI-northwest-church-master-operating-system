...
export function OfflineReadiness() {
  ...
  const [message, setMessage] = useState("Save the public service schedule and approved weekly lesson summary for low-connectivity Sundays.");
  ...
  async function save() {
    try {
      ...
      if (responses.some((response) => !response.ok)) throw new Error("Offline snapshot request failed");
  ...
  return (
    <section className="hub-panel notification-capability" aria-live="polite">
      <div>
        ...
        <p>{message}</p>
        <small>Community messages, household information, child status, and prayer requests are never cached for offline use.</small>
      </div>
