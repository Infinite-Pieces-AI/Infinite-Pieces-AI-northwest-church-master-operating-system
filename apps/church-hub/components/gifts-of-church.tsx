...
const previewPosts: GiftPost[] = [
  ...
  },
  {
    ...
    description:
      "A household in the region could use one person with basic tools for about an hour. No child-care or home access is shared until both members agree privately.",
...
const storageKey = "church-hub-gifts-showcase-v1";
...
export function GiftsOfChurch({
  ...
  const [activeTab, setActiveTab] = useState<"board" | "gifts" | "create" | "matches">("board");
  ...
  const matchedPosts = useMemo(() => {
    const strongest = new Set(scores.filter((score) => score.score >= 50).map((score) => score.label));
    return posts
      .map((post) => ({
        post,
  ...
  function guide() {
    ...
    if (/assessment|score|gift|wired|strength/.test(text)) {
      ...
      setActiveTab("create");
      setNotice("Opened Create so you can offer a skill, request help, share an item, or post an approved church need.");
    } else {
      setActiveTab("matches");
      setNotice("Opened Matches based only on the gift labels you explicitly entered—not private messages or inferred spirituality.");
    }
  }
  ...
  async function createPost(event: React.FormEvent<HTMLFormElement>) {
    ...
    const tags = String(data.get("giftTags") ?? "")
      .split(",")
      .map((value) => value.trim())
      .filter(Boolean)
      .slice(0, 12);
    const skills = String(data.get("skillTags") ?? "")
      .split(",")
      .map((value) => value.trim())
      .filter(Boolean)
      .slice(0, 12);
  ...
  async function respond(post: GiftPost) {
    const message = window.prompt("Write a short private response to the post owner:");
    ...
    try {
      if (mode === "showcase") {
        ...
        await sendLive("respond", { postId: post.id, message: message.trim() });
      }
          ...
          <div className="gift-board-grid">
            {visiblePosts.map((post) => (
              <article className="gift-post-card" key={post.id}>
                ...
                <dl>
                  <div><dt>Posted by</dt><dd>{post.ownerName}</dd></div>
                  {post.generalLocation ? <div><dt>Area</dt><dd>{post.generalLocation}</dd></div> : null}
                  {post.availability ? <div><dt>When</dt><dd>{post.availability}</dd></div> : null}
                  {post.priceNote ? <div><dt>Terms</dt><dd>{post.priceNote}</dd></div> : null}
                </dl>
                <footer>
                  <button type="button" onClick={() => void respond(post)}>Respond privately</button>
                  <small>{post.responses.length} response{post.responses.length === 1 ? "" : "s"}</small>
                </footer>
              </article>
            ))}
          </div>
      ...
      {!loading && activeTab === "matches" ? (
        <section className="module-workspace">
          <div className="section-heading">
            <div><p>Explainable matching</p><h3>Needs that overlap with your entered gifts</h3></div>
          </div>
          ...
          <div className="match-list">
            {matchedPosts.map(({ post, score }) => (
              <article key={post.id}>
                ...
                <span>{score} matching gift tag{score === 1 ? "" : "s"}: {post.giftTags.filter((tag) => scores.some((gift) => gift.label === tag && gift.score >= 50)).join(", ")}</span>
                <button type="button" onClick={() => void respond(post)}>I may be able to help</button>
              </article>
            ))}
          </div>
        </section>
      ...
      {!loading && activeTab === "gifts" ? (
        <section className="module-workspace gifts-report">
          <div className="section-heading">
            ...
            {assessmentUrl ? <a href={assessmentUrl} target="_blank" rel="noreferrer">Open approved assessment ↗</a> : null}
          </div>
          <p className="module-boundary">
          ...
          <div className="gift-score-list">
            {scores.map((score) => (
              <label key={score.id}>
                <span><strong>{score.label}</strong><small>{themeLabels[score.theme]}</small></span>
                <div><i style={{ width: `${score.score}%` }} data-theme={score.theme} /></div>
                <input
                  type="number"
                  min={0}
                  max={100}
          ...
          {mode === "showcase" ? <button className="module-secondary" type="button" onClick={resetShowcase}>Reset showcase</button> : null}
        </section>
      ) : null}

      {!loading && activeTab === "create" ? (
        <section className="module-workspace">
          <div className="section-heading"><div><p>One clear post</p><h3>Offer a gift or describe a need</h3></div></div>
          <form className="module-form" onSubmit={(event) => void createPost(event)}>
            <label>Post type<select name="type" defaultValue="offer">
            ...
            <label className="span-2">Description<textarea name="description" required minLength={10} maxLength={5000} rows={5} /></label>
            <label>Spiritual-gift tags<input name="giftTags" placeholder="Hospitality, Mercy, Leadership" /></label>
            <label>Practical skill tags<input name="skillTags" placeholder="Cooking, tutoring, repairs" /></label>
            <label>Exchange<select name="exchangeType" defaultValue="free">
            ...
            <label>Terms, if paid<input name="priceNote" placeholder="The app does not process payment" /></label>
            <label>General area<input name="generalLocation" placeholder="Lowell area; exact address shared privately" /></label>
            <label>Availability<input name="availability" placeholder="Saturday mornings" /></label>
            <button type="submit">Publish to approved members</button>
          </form>
          <p className="module-boundary">
            The church may moderate posts. The app does not process payments, verify professional
            licenses, authorize unscreened child care, or permit dangerous, illegal, or restricted items.
          </p>
        </section>
      ) : null}
    </div>
