...
interface ServiceLocation {
  ...
  name: string;
  listingKind:
    "church_site" | "approved_partner" | "public_lead" | "public_place" | "self_guided_area";
  ...
  proposals: [
    {
      ...
      needStatement:
        "A public trail area has visible litter and could benefit from a small, supervised cleanup team.",
      impactStatement:
        "The project could leave the area cleaner while helping families practice neighborhood service together.",
      category: "Environment",
      kind: "member_led",
    ...
    },
    {
      ...
      impactStatement:
        "Skilled volunteers could reduce a practical burden if the safety and home-access plan is approved.",
      category: "Housing",
      kind: "member_led",
  ...
  impacts: [
    {
      ...
      summary:
        "The team packed and quality-checked kits for delivery through an approved partner pathway. No recipient names or photographs were collected.",
      peopleServed: 48,
      volunteerCount: 21,
      hoursServed: 26.25,
...
export function ServiceAdmin({
  ...
  async function sendLive(action: string, values: Record<string, unknown>) {
    ...
    if (!response.ok)
      throw new Error(result.message ?? "The service administration action failed.");
    await refreshLive();
  }

  const metrics = useMemo(
    () => ({
      published: payload.opportunities.filter((item) => item.publicationStatus === "published")
        .length,
      ...
      locationsToReview: payload.locations.filter((item) => item.reviewStatus === "research")
        .length,
    }),
    [payload],
  );
  ...
  async function reviewProposal(
    ...
    decision: "approved" | "needs_changes" | "declined",
  ) {
          ...
          const newOpportunity: AdminOpportunity | null =
            decision === "approved"
              ? {
                  ...
                  kind: proposal.kind === "approved_partner" ? "public_lead" : proposal.kind,
  ...
  async function createOpportunity(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice(
        "Draft opportunity created. Review sponsorship, safety, location, and shift details before publishing.",
      );
    } catch (error) {
  ...
  async function createLocation(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset();
      setNotice(
        "Location added to research. It is not yet an approved church partner or confirmed opening.",
      );
    } catch (error) {
  ...
  async function updateLocation(
    ...
    reviewStatus: ServiceLocation["reviewStatus"],
  ) {
    try {
      if (mode === "showcase") {
        setPayload((current) => ({
  ...
  return (
    <div className="service-admin">
      <section className="service-admin__metrics">
        ...
        <article>
          <strong>{metrics.openCapacity}</strong>
          <span>open volunteer spaces</span>
        </article>
        <article>
          <strong>{metrics.pendingProposals}</strong>
          <span>member ideas awaiting review</span>
        </article>
        <article>
          <strong>{metrics.locationsToReview}</strong>
          <span>locations needing verification</span>
        </article>
      </section>
      ...
      {!loading && activeTab === "opportunities" ? (
        <section className="module-workspace">
          <div className="section-heading">
            <div>
              <p>Source of truth</p>
              <h3>Service opportunity publishing pipeline</h3>
            </div>
          </div>
            ...
            {payload.opportunities.map((opportunity) => (
              <article key={opportunity.id}>
                ...
                <div>
                  ...
                  <small>
                    {opportunity.churchSponsored ? "Church-sponsored" : "Not church-sponsored"}
                  </small>
                </div>
                <div>
                  <span>{formatDate(opportunity.nextShift)}</span>
                  <small>
                    {opportunity.capacity
                      ? `${opportunity.signupCount}/${opportunity.capacity} registered`
                      : "External or unscheduled"}
                  </small>
                </div>
                <div>
                  <b className={`admin-status admin-status--${opportunity.publicationStatus}`}>
                    {opportunity.publicationStatus.replaceAll("_", " ")}
                  </b>
                  <small>{opportunity.visibility}</small>
                </div>
                <div>
                  {opportunity.publicationStatus !== "published" ? (
                    <button type="button" onClick={() => void publishOpportunity(opportunity)}>
                      Publish
                    </button>
                  ) : (
                    <a href={`/serve?opportunity=${opportunity.id}`}>View →</a>
                  )}
                </div>
              </article>
            ))}
          ...
          <div className="service-admin-proposal-list">
            {payload.proposals.map((proposal) => (
              <button
                type="button"
                key={proposal.id}
                className={selectedProposal === proposal.id ? "active" : ""}
                onClick={() => setSelectedProposal(proposal.id)}
              >
                <span className={`risk-badge risk-badge--${proposal.riskLevel}`}>
                  {proposal.riskLevel}
                </span>
                <strong>{proposal.title}</strong>
                <small>
                  {proposal.memberName} · {proposal.status.replaceAll("_", " ")}
                </small>
              </button>
            ))}
          </div>
          <article className="service-admin-proposal-detail">
            {payload.proposals.find((proposal) => proposal.id === selectedProposal) ? (
              (() => {
                ...
                return (
                  <>
                    ...
                    <p>
                      <strong>Submitted by:</strong> {proposal.memberName}
                    </p>
                    <p>
                      <strong>Location:</strong> {proposal.generalLocation}
                      {proposal.postalCode ? ` · ${proposal.postalCode}` : ""}
                    </p>
                    <h4>Need</h4>
                    <p>{proposal.needStatement}</p>
                    <h4>Expected impact</h4>
                    <p>{proposal.impactStatement}</p>
                    <h4>Risk flags</h4>
                    ...
                    {proposal.reviewerNote ? (
                      <div className="module-boundary">{proposal.reviewerNote}</div>
                    ) : null}
                    {proposal.status === "pending" || proposal.status === "needs_changes" ? (
                      <div className="service-admin-review-actions">
                        <button
                          type="button"
                          className="success"
                        ...
                        <button
                          type="button"
                          onClick={() => void reviewProposal(proposal, "needs_changes")}
                        >
                          Request changes
                        </button>
                        <button
                          type="button"
                          className="danger"
                          onClick={() => void reviewProposal(proposal, "declined")}
                        >
                          Decline
                        </button>
                      </div>
              ...
              <p className="module-empty">
                Choose a member proposal to review its need, impact, location, and safety flags.
              </p>
            )}
          </article>
        </section>
      ...
      {!loading && activeTab === "locations" ? (
        <section className="service-location-layout">
          <div className="service-location-list">
            {payload.locations.map((location) => (
              <article key={location.id}>
                <header>
                  <span>{location.listingKind.replaceAll("_", " ")}</span>
                  <b>{location.reviewStatus.replaceAll("_", " ")}</b>
                </header>
                <h3>{location.name}</h3>
                <p>
                  {location.locality}
                ...
                {location.publicUrl ? (
                  <a href={location.publicUrl} target="_blank" rel="noreferrer">
                    Open public source ↗
                  </a>
                ) : null}
                <small>
                  {location.sourceVerifiedAt
                    ? `Source reviewed ${new Date(location.sourceVerifiedAt).toLocaleDateString()}`
                    : "Source has not been verified"}
                </small>
                <footer>
                  <button
                    type="button"
                    onClick={() => void updateLocation(location, "public_lead")}
                  >
                    Keep as public lead
                  </button>
                  <button
                    type="button"
                    className="success"
                    onClick={() => void updateLocation(location, "approved")}
                  >
                    Approve location
                  </button>
                  <button
                    type="button"
                    className="danger"
                    onClick={() => void updateLocation(location, "do_not_use")}
                  >
                    Do not use
                  </button>
                </footer>
          ...
          <form
            className="module-form service-location-form"
          ...
          >
            ...
            <label>
              Listing type
              <select name="listingKind" defaultValue="public_lead">
                <option value="public_lead">Public lead</option>
                <option value="approved_partner">Possible partner</option>
                <option value="public_place">Public place</option>
                <option value="church_site">Church site</option>
                <option value="self_guided_area">Self-guided area</option>
              </select>
            </label>
            <label>
              Organization type
              <select name="organizationType" defaultValue="other">
            ...
            <label>
              Locality
              <input name="locality" required placeholder="Lowell" />
            </label>
            <label>
              ZIP code
            ...
            <small>
              A public listing is not automatically a church partner or current volunteer opening.
            </small>
          </form>
        </section>
      ) : null}
      ...
      {!loading && activeTab === "impact" ? (
        <section className="module-workspace">
          <div className="section-heading">
            <div>
              <p>Dignity before publicity</p>
              <h3>Review service impact without exposing people receiving support</h3>
            </div>
          </div>
          <div className="service-impact-grid">
            {payload.impacts.map((impact) => (
              <article key={impact.id}>
                <span>{impact.opportunityTitle}</span>
                <h3>{impact.headline}</h3>
                <p>{impact.summary}</p>
                <dl>
                  <div>
                    <dt>People served</dt>
                    <dd>{impact.peopleServed ?? "Not recorded"}</dd>
                  </div>
                  <div>
                    <dt>Volunteers</dt>
                    <dd>{impact.volunteerCount ?? "Not recorded"}</dd>
                  </div>
                  <div>
                    <dt>Hours</dt>
                    <dd>{impact.hoursServed ?? "Not recorded"}</dd>
                  </div>
                </dl>
                <footer>
                  <b>
                    {impact.approvedForMembers ? "Approved for members" : "Member approval pending"}
                  </b>
                  <b>{impact.approvedForPublic ? "Approved for public" : "Not public"}</b>
                </footer>
              </article>
            ))}
          </div>
          <p className="module-boundary">
            Do not publish recipient names, faces, exact private addresses, immigration information,
            medical details, recovery participation, prayer content, or hardship narratives without
            the appropriate written consent and ministry review.
          </p>
        </section>
      ) : null}
        ...
        <section className="module-workspace">
          <form className="module-form" onSubmit={(event) => void createOpportunity(event)}>
            <div className="section-heading span-2">
              <div>
                <p>Draft first</p>
                <h3>Create an official service opportunity</h3>
              </div>
            </div>
            <label>
              Title
            ...
            <label>
              Partner or host
              <input name="partnerName" required minLength={2} maxLength={160} />
            </label>
            <label>
              Ownership
              <select name="kind" defaultValue="church_hosted">
                <option value="church_hosted">Church-hosted</option>
                <option value="approved_partner">Approved partner</option>
                <option value="member_led">Member-led · not church-sponsored</option>
                <option value="self_guided">Self-guided</option>
                <option value="public_lead">Public lead · verification required</option>
              </select>
            </label>
            <label>
              Audience
              <select name="visibility" defaultValue="members">
                ...
                <option value="public">Public website and members</option>
              </select>
            </label>
            <label className="span-2">
              Need statement
              <textarea name="needStatement" rows={4} required minLength={20} maxLength={2000} />
            </label>
            <label className="span-2">
              Impact statement
              <textarea name="impactStatement" rows={4} required minLength={20} maxLength={2000} />
            </label>
            <label>
              Category
              <input name="category" required placeholder="Hunger and food access" />
            </label>
            <label>
              General location
              <input name="generalLocation" required maxLength={200} />
            </label>
            <label>
              ZIP code
              <input name="postalCode" inputMode="numeric" maxLength={5} />
            ...
            <label>
              Age requirements
              <input name="ageRequirements" required defaultValue="Adults" />
            </label>
            <label className="span-2">
              Accessibility notes
              <textarea name="accessibilityNotes" rows={3} />
            </label>
            <label className="span-2">
              Safety and safeguarding summary
              <textarea name="safetySummary" rows={4} required />
            </label>
            <label>
              First shift begins
            ...
            <label>
              Capacity
              <input name="capacity" type="number" min={1} max={1000} defaultValue={20} />
            </label>
      ...
      {mode === "showcase" ? (
        <button type="button" className="module-secondary" onClick={resetShowcase}>
          Reset service administration showcase
        </button>
      ) : null}
    </div>
  );
