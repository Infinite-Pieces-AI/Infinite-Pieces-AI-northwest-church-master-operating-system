...
export function RecoveryAdminConsole({ mode }: { mode: "showcase" | "live" }) {
    ...
    if (mode === "showcase") {
      ...
      if (stored) {
        try {
          ...
          if (Array.isArray(parsed.programs) && Array.isArray(parsed.accessRequests)) setPayload(parsed);
        } catch {
  ...
  async function load() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.message ?? "Recovery administration could not be loaded.");
      setPayload(result);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Recovery administration could not be loaded.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="recovery-admin-console">
      <nav className="module-tabs" aria-label="Recovery administration sections">
        <button type="button" className={tab === "programs" ? "active" : ""} onClick={() => setTab("programs")}>Programs</button>
        <button type="button" className={tab === "access" ? "active" : ""} onClick={() => setTab("access")}>Access requests</button>
        <button type="button" className={tab === "create" ? "active" : ""} onClick={() => setTab("create")}>Create program</button>
      </nav>
        ...
        <section className="recovery-admin-programs">
          {payload.programs.map((program) => (
            <article key={program.id}>
              <header><span>{program.programType.replaceAll("_", " ")}</span><b>{program.status}</b></header>
              <h3>{program.displayName}</h3><p>{program.publicSummary}</p>
              <dl><div><dt>Meeting</dt><dd>{program.meetingDay ?? "Not published"} · {program.meetingTime ?? "Time private"}</dd></div><div><dt>Participants</dt><dd>{program.participantCount}</dd></div><div><dt>Published weeks</dt><dd>{program.sessionCount}</dd></div><div><dt>Official program confirmation</dt><dd>{program.officialProgramConfirmation ? "Recorded" : "Not claimed"}</dd></div></dl>
              <select value={program.status} onChange={(event) => void changeProgramStatus(program, event.target.value)}><option value="draft">Draft</option><option value="active">Active</option><option value="paused">Paused</option><option value="retired">Retired</option></select>
            </article>
          ))}
          {!payload.programs.length ? <p className="module-empty">No private recovery program is configured.</p> : null}
        </section>
      ) : null}
        ...
        <section className="recovery-access-queue">
          {payload.accessRequests.map((request) => (
            <article key={request.id}>
              <header><strong>{request.profileName}</strong><span>{request.status}</span></header>
              <p>{request.programName} · {new Date(request.createdAt).toLocaleString()}</p>
              {request.requestMessage ? <blockquote>{request.requestMessage}</blockquote> : null}
              {request.status === "pending" ? <div><button type="button" onClick={() => void review(request, "approved")}>Approve private access</button><button type="button" onClick={() => void review(request, "declined")}>Decline or redirect</button></div> : null}
            </article>
          ))}
          {!payload.accessRequests.length ? <p className="module-empty">No recovery access requests are waiting.</p> : null}
          <p className="module-boundary">Approval creates a private participant membership. Ordinary members cannot enroll themselves, see the roster, or discover another member’s request.</p>
        </section>
      ) : null}
      ...
      {!loading && tab === "create" ? (
        <section className="module-workspace">
          <form className="module-form" onSubmit={(event) => void createProgram(event)}>
            ...
            <label>Public ministry name<input name="displayName" defaultValue="Recovery Ministry" required maxLength={160} /></label>
            <label>Meeting day<input name="meetingDay" placeholder="Sunday" maxLength={40} /></label>
            <label>Meeting time<input name="meetingTime" type="time" /></label>
            <label>General location<input name="generalLocation" placeholder="Lowell · exact room private" maxLength={200} /></label>
            <label className="span-2">Public summary<textarea name="publicSummary" rows={5} required minLength={20} maxLength={3000} /></label>
            <label className="check-label span-2"><input name="officialProgramConfirmation" type="checkbox" /> Leadership has confirmed an official Celebrate Recovery relationship and permission to use its name and curriculum.</label>
            <button type="submit">Create program and leader access</button>
          </form>
          <p className="module-boundary">Do not select official program confirmation merely because the church discusses recovery. Confirm the program relationship, curriculum permission, branding, and testimony/copyright rules first.</p>
        </section>
      ) : null}
      {mode === "showcase" ? <button type="button" className="module-secondary" onClick={resetShowcase}>Reset admin showcase</button> : null}
    </div>
  );
}
