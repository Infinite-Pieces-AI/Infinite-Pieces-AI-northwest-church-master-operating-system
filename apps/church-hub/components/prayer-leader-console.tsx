...
export function PrayerLeaderConsole() {
  ...
  async function load() {
    ...
    try {
      ...
      const payload = (await response.json()) as { requests?: RestrictedPrayer[]; message?: string };
      if (!response.ok) throw new Error(payload.message ?? "The restricted prayer queue could not be loaded.");
      setRequests(payload.requests ?? []);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "The restricted prayer queue could not be loaded.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function update(request: RestrictedPrayer, workflowStatus: string) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.message ?? "The restricted prayer workflow could not be updated.");
      setNotice("Restricted prayer workflow updated.");
      await load();
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "The restricted prayer workflow could not be updated.");
    }
  }
        ...
        <div className="prayer-leader-list">
          {requests.map((request) => (
            <article key={request.id}>
              <header>
                <div><span>{request.sensitivity}</span><span>{request.workflowStatus.replaceAll("_", " ")}</span></div>
                <small>{new Date(request.createdAt).toLocaleString()}</small>
              </header>
              <h4>{request.title}</h4>
              <p>{request.requestText}</p>
              <dl>
                <div><dt>Owner</dt><dd>{request.ownerName}</dd></div>
                <div><dt>Member-feed display</dt><dd>{request.displayAnonymous ? "Anonymous if later permitted" : "Named if later permitted"}</dd></div>
                <div><dt>Category</dt><dd>{request.category}</dd></div>
                <div><dt>Assigned to</dt><dd>{request.assignedTo ?? "Unassigned"}</dd></div>
              </dl>
              {request.leaderNote ? <blockquote>{request.leaderNote}</blockquote> : null}
              <div>
                <button type="button" onClick={() => void update(request, "assigned")}>Assign to me</button>
                <button type="button" onClick={() => void update(request, "pastoral_followup")}>Pastoral follow-up</button>
                <button type="button" onClick={() => void update(request, "safeguarding_followup")}>Safeguarding follow-up</button>
                <button type="button" onClick={() => void update(request, "closed")}>Close workflow</button>
              </div>
            </article>
          ))}
          {!requests.length ? <p className="module-empty">No restricted prayer requests match this queue.</p> : null}
        </div>
      ) : null}
    </section>
  );
