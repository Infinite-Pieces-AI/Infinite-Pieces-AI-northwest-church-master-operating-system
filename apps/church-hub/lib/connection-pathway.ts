...
export interface ConnectionPathwayState {
  status: "active" | "paused" | "completed" | "archived";
  steps: Record<ConnectionStepKey, ConnectionStepStatus>;
}

export async function loadConnectionPathway(viewer: Viewer): Promise<ConnectionPathwayState> {
  const fallback: ConnectionPathwayState = {
    status: "active",
    steps: {
      visit: "not_started",
      fellowship: "not_started",
      bible: "not_started",
      service: "not_started",
    },
  };
  ...
  for (const step of steps ?? []) {
    if (["visit", "fellowship", "bible", "service"].includes(String(step.step_key)))
      mapped[String(step.step_key) as ConnectionStepKey] = String(
        step.status,
      ) as ConnectionStepStatus;
  }
  return {
    status: enrollment?.status
      ? (String(enrollment.status) as ConnectionPathwayState["status"])
      : "active",
    steps: mapped,
  };
}
