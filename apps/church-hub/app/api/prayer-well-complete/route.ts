...
async function loadAudiences(client: SupabaseClient, viewerId: string) {
  const [groupsResult, ministriesResult] = await Promise.all([
    client
    ...
    client
      ...
      .select("ministry_id,ministry:ministries(id,name)")
      .eq("profile_id", viewerId),
  ]);
  const groups = groupsResult.error
    ? []
    : ((groupsResult.data ?? []) as Row[]).flatMap((membership) => {
        const group = relation(membership.group);
        const id = String(group?.id ?? membership.group_id ?? "");
        const name = String(group?.name ?? "Assigned group");
        return id ? [{ id, name }] : [];
      });
  const ministries = ministriesResult.error
    ? []
    : ((ministriesResult.data ?? []) as Row[]).flatMap((membership) => {
        const ministry = relation(membership.ministry);
        const id = String(ministry?.id ?? membership.ministry_id ?? "");
        const name = String(ministry?.name ?? "Assigned ministry");
        return id ? [{ id, name }] : [];
      });
...
async function loadPayload(client: SupabaseClient, viewerId: string) {
  ...
  const [ownerResult, interactionResult, audiences] = await Promise.all([
    ids.length
    ...
    requests: requests.map((request) => ({
      ...
      audienceId:
        typeof request.group_id === "string"
          ? request.group_id
          : typeof request.ministry_id === "string"
            ? request.ministry_id
            : undefined,
