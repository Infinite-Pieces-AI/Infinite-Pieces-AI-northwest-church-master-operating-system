...
const schema = z.object({
  ...
  transportationNote: z.string().trim().max(500).optional(),
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success)
    return NextResponse.json({ message: "Check the shift response." }, { status: 400 });
  if (viewer.demo)
    return NextResponse.json({ status: parsed.data.status, count: null, mode: "demo" });
    ...
    .upsert(
      {
        ...
        transportation_note: parsed.data.transportationNote || null,
        accessibility_note: parsed.data.accessibilityNote || null,
  ...
  if (error || !data)
    return NextResponse.json(
      { message: "The service shift response could not be saved." },
      { status: 403 },
    );
