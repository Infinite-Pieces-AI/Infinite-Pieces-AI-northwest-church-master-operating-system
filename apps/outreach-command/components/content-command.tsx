...
export function ContentCommand() {
  ...
  const [audience, setAudience] = useState(
    "Adults voluntarily looking for an online Bible conversation",
  );
  const [draft, setDraft] = useState<string | null>(null);
  const [queue, setQueue] = useState<QueueItem[]>([...contentQueue]);
  const facts = useMemo(
    () => canonicalFacts.map((fact) => `${fact.label}: ${fact.value}`).join(" · "),
    [],
  );
  ...
  function advance(id: string) {
    setQueue((current) =>
      current.map((item) => {
        ...
        const next =
          status === "Draft" ? "In review" : status === "In review" ? "Approved" : status;
        ...
        <section className="panel">
          <div className="panel__header">
            <div>
              ...
              <p>Deterministic demo generation from approved facts and a stated public need.</p>
            </div>
          </div>
          <div className="panel__body">
            ...
            <div className="field-grid">
              <label className="field field--span2">
                Public question or content goal
                <input value={intent} onChange={(event) => setIntent(event.target.value)} />
              </label>
              <label className="field field--span2">
                Intended public audience and need
                <textarea
                  rows={4}
                  value={audience}
            ...
            <p className="notice notice--gold">
              The demo never publishes. Production requires approved church facts, a named reviewer,
        ...
        <section className="panel">
          <div className="panel__header">
            <div>
              ...
              <p>Separated from live public content until approval.</p>
            </div>
          </div>
          <div className="panel__body">
            {draft ? (
              <div className="generated-document">
                ...
                <div className="approval-rail">
                  ...
                  <article>
                    ...
                    <small>When applicable</small>
                  </article>
                  <article>
                    <strong>Publish</strong>
                    <small>Named approver</small>
                  </article>
                </div>
              </div>
            ) : (
              <div className="empty-state">
                ...
                <p>
                  Select a content type and state the public question the church is trying to
                  answer.
                </p>
              </div>
            )}
      ...
      <section className="panel" style={{ marginTop: 18 }}>
        <div className="panel__header">
          <div>
            ...
            <p>Publishing and replies remain disabled until the exact version receives approval.</p>
          </div>
        </div>
              ...
              {queue.map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span
                        className={`status-pill status-pill--${status === "Approved" ? "approved" : "review"}`}
                      >
                        {status}
                      </span>
                    </td>
                    <td>{item.owner}</td>
                    <td>{item.theologicalReview ? "Required" : "Not required"}</td>
                    <td>{item.updated}</td>
                    <td>
                      <button
