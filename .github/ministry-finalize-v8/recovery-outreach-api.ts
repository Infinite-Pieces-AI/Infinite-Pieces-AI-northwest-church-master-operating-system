...
async function loadPayload(client: SupabaseClient) {
  const [partnersResult, topicsResult, inquiriesResult] = await Promise.all([
    ...
    client
      ...
      .select(
        "id,source_kind,topic,locality,public_url,aggregate_impressions,aggregate_clicks,opportunity_score,sensitivity_score,recommended_action,status",
      )
    ...
    client
      ...
      .select(
        "id,first_name,email,phone,preferred_contact,requested_next_step,message,source_path,status,assigned_to,consented_at,created_at",
      )
  ...
  return {
    ...
    counts: {
      ...
      approvedPartners: partners.filter(
...
async function authorizedViewer() {
  ...
  return viewer && canManageRecoveryOutreach(viewer.roles) ? viewer : null;
}
  ...
  try {
    if (action === "add_partner") {
      ...
      if (!partnerStatuses.has(status)) {
        throw new Error("Unsupported partnership status.");
      }
      ...
      if (!sourceKinds.has(sourceKind)) {
        throw new Error("Unsupported public source type.");
      }
